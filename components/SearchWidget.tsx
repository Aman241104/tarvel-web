...
Can you help me with the itinerary?`;
    ...
    return (
        ...
        >
            <div className="bg-white/90 backdrop-blur-2xl border border-white/40 rounded-[2rem] md:rounded-full p-2 md:p-3 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] flex flex-col md:flex-row items-stretch md:items-center gap-1 md:gap-0 relative">
                
                {/* Concierge Badge */}
                <div className="absolute -top-9 left-6 md:left-12 bg-white/60 backdrop-blur-xl px-3 py-1 rounded-full border border-white/50 text-[9px] font-black uppercase tracking-widest text-text-navy/40 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-teal animate-pulse" />
                    Concierge Online
                </div>

                {/* Location Input - Full width on mobile */}
                <div className={`relative flex-[1.5] p-3 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'location' ? 'bg-white shadow-lg scale-[1.01] z-10' : 'hover:bg-white/40'}`}>
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="flex flex-col w-full text-left">
                            ...
                            <input
                                ...
                                className="bg-transparent outline-none text-text-navy font-black text-xs md:text-base placeholder:text-text-navy/30 w-full"
                                value={location}
                ...
                <div className="flex md:contents gap-1">
                    {/* Date Input */}
                    <div className={`relative flex-1 p-3 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'date' ? 'bg-white shadow-lg scale-[1.01] z-10' : 'hover:bg-white/40'}`}>
                        <div className="flex items-center gap-2 md:gap-4">
                            <div className="bg-brand-coral/5 p-1.5 md:p-2 rounded-full shrink-0">
                    ...
                    {/* Guests Input */}
                    <div className={`relative flex-1 p-3 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'guests' ? 'bg-white shadow-lg scale-[1.01] z-10' : 'hover:bg-white/40'}`}>
                        <div className="flex items-center gap-2 md:gap-4">
                            <div className="bg-brand-yellow/5 p-1.5 md:p-2 rounded-full shrink-0">
                ...
                <button
                    ...
                    className="bg-text-navy hover:bg-brand-coral text-white p-3.5 md:p-5 rounded-2xl md:rounded-full shadow-2xl transition-all duration-700 hover:scale-105 active:scale-95 mt-1 md:mt-0 w-full md:w-auto flex justify-center items-center gap-3 md:min-w-[200px] group relative overflow-hidden"
                >
