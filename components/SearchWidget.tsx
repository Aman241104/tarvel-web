            ...
            <div className="bg-white/95 backdrop-blur-2xl border border-black/5 rounded-[2rem] md:rounded-full p-3 md:p-3 shadow-[0_40px_80px_-15px_rgba(0,0,0,0.15)] flex flex-col md:flex-row items-stretch md:items-center gap-2 md:gap-0 relative">
                ...
                <div className={`relative flex-[1.5] p-2.5 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'location' ? 'bg-brand-teal/5 shadow-[inset_0_0_0_2px_rgba(46,196,182,0.2)] scale-[1.01] z-10' : 'hover:bg-black/5'}`}>
                    <div className="flex items-center gap-3 md:gap-4">
                        <div className={`transition-all duration-500 p-2 md:p-2.5 rounded-full shrink-0 ${activeInput === 'location' ? 'bg-brand-teal text-white' : 'bg-brand-teal/10 text-brand-teal'}`}>
                            <MapPin className="w-4 h-4 md:w-5 md:h-5" aria-hidden="true" />
                        </div>
                        <div className="flex flex-col w-full text-left">
                            <label htmlFor="location-input" className={`text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] mb-0.5 md:mb-1 cursor-pointer transition-colors duration-300 ${activeInput === 'location' ? 'text-brand-teal' : 'text-brand-teal/60'}`}>Where to?</label>
                            <input
                                id="location-input"
                                type="text"
                ...
                <div className="flex flex-col md:flex-row w-full md:w-auto md:flex-2 gap-2 md:gap-0">
                    {/* Date Input */}
                    <div className={`relative flex-1 p-2.5 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'date' ? 'bg-brand-coral/5 shadow-[inset_0_0_0_2px_rgba(255,107,107,0.2)] scale-[1.01] z-10' : 'hover:bg-black/5'}`}>
                        <div className="flex items-center gap-3 md:gap-4">
                            <div className={`transition-all duration-500 p-2 md:p-2.5 rounded-full shrink-0 ${activeInput === 'date' ? 'bg-brand-coral text-white' : 'bg-brand-coral/10 text-brand-coral'}`}>
                                <Calendar className="w-4 h-4 md:w-5 md:h-5" aria-hidden="true" />
                            </div>
                            <div className="flex flex-col w-full text-left">
                                <label htmlFor="date-input" className={`text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] mb-0.5 md:mb-1 cursor-pointer transition-colors duration-300 ${activeInput === 'date' ? 'text-brand-coral' : 'text-brand-coral/60'}`}>When?</label>
                                <input
                                    id="date-input"
                                    type="text"
                    ...
                    {/* Guests Input */}
                    <div className={`relative flex-1 p-2.5 md:p-4 rounded-2xl md:rounded-full transition-all duration-500 ${activeInput === 'guests' ? 'bg-brand-yellow/5 shadow-[inset_0_0_0_2px_rgba(255,191,0,0.2)] scale-[1.01] z-10' : 'hover:bg-black/5'}`}>
                        <div className="flex items-center gap-3 md:gap-4">
                            <div className={`transition-all duration-500 p-2 md:p-2.5 rounded-full shrink-0 ${activeInput === 'guests' ? 'bg-brand-yellow text-white' : 'bg-brand-yellow/10 text-brand-yellow'}`}>
                                <Users className="w-4 h-4 md:w-5 md:h-5" aria-hidden="true" />
                            </div>
                            <div className="flex flex-col w-full text-left">
                                <label htmlFor="guests-input" className={`text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] mb-0.5 md:mb-1 cursor-pointer transition-colors duration-300 ${activeInput === 'guests' ? 'text-brand-yellow' : 'text-brand-yellow/60'}`}>Who?</label>
                                <input
                                    id="guests-input"
                                    type="text"
